process SPLIT_BAM {
    ...
    samtools view -@ \$threads -h -d HP:1 -L ${regions_bed} ${bam} -o ${sampleid}.HP1.bam
    samtools index -@ \$threads ${sampleid}.HP1.bam

    # Split haplotype 2 (HP:i:2)
    samtools view -@ \$threads -h -d HP:2 -L ${regions_bed} ${bam} -o ${sampleid}.HP2.bam
    samtools index -@ \$threads ${sampleid}.HP2.bam

    # Extract untagged reads (no HP tag)
    samtools view -@ \$threads -h -e '!([HP])' -L ${regions_bed} ${bam} -o ${sampleid}.untagged.bam
